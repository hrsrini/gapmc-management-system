...
| **P1 #49** | **`GET /api/ioms/reports/tally-export`** — `format=csv` (legacy + `columns=srs`), **`format=xml`** → `gapmcTallyExport` v1 (`server/tally-export-xml.ts`); **`tally_xml_export_enabled`**; deposit ref in SRS narration/XML | Native Tally Prime **import** XML without CA transform |
| **M-05 §8.4** | **Receipt deposit & settlement** (FR-RCP-010–014): bank accounts + version history, deposit entry, cash-in-hand + defer + EOD/overdue cron, DV/DA register (verify/approve/reject/reverse), cheque dishonour after deposit, Tally deposit ref; M-03 ledger gated until DA approve | Finance UAT on Tally narration; production DB migrations 051/052; configure NOTIFY for EOD |
...
| P1 | 49 | §8.3 FR-RCP-008 | Tally export | CSV + interchange **XML** + admin toggle + **deposit ref** on settled receipts | [Q49 pending](./CLIENT-CLARIFICATION-PENDING.md); [Responses Q49](./CLIENT-CLARIFICATION-RESPONSES-2026.md) | Eng | UAT with finance / Tally Prime |
...
| Receipt deposit §8.4 | `server/receipt-deposit-service.ts`, `server/routes-receipt-deposits.ts`, `server/cron-receipt-deposit.ts`, migrations `051`/`052`, client `/receipts/ioms/cash-in-hand`, `/deposit-entry`, `/deposits`, `/admin/bank-accounts` |
...
*Last updated: 2026-05-19 — refresh when SRS, Excel, or repo behaviour changes.*
